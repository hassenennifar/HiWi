...
% Kalman parameters
P.Pk = diag(repmat([1e1; 1e2], P.nj,1));
P.Qk = P.Pk;
P.Rk = 1e-4;
...
% mat = load('Duhamel_CC_CV_2.5C.mat');
% ts = mat.ts(1,2:end);
% ym = mat.v(1,2:end);
% I = mat.cur(1,2:end);
...
%     ts(k) = tvec(k);
    ...
    P.Qk = diag(repmat([(0.6*P.dt*cur/P.F/P.L_neg/P.epsl_neg)^2; ...
        (3*P.dt*cur/P.F/P.Rp_neg)^2], P.nj,1));% ...
        %0*(45/2*P.dt*cur/P.F/P.Rp_pos^2)^2 % process noise cov
%     if abs(cur) < 2.5*21.5
%         P.Rk = 1e-4;
%     else
...
%     end
    ...
    sol = approxSolid_Eigen(k,P,x,sol);
...
% xm = mat.x;
% Pm = mat.P;
% cssm = xm(Pm.idx_il:Pm.nx:Pm.nx*P.nj, :);
% csse = x_est(P.idx_il:P.nx:P.nx*P.nj, :);
% plot(ts, cssm(3,2:end)/Pm.csmax_neg, 'b')
% hold on
% plot(ts, csse(3,:)/P.csmax_neg, 'k--')

% plot(ts, cssm([Pm.bnd_pos_sep Pm.nj],2:end)/Pm.csmax_pos, '.b')
% plot(ts, csse([P.bnd_pos_sep P.nj],:)/P.csmax_pos, 'k.--')