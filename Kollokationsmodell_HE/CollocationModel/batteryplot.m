function []=batteryplot(result,plots)
...
% choose evaluation times: for now simply choose N time steps close to
% equally spaced in simulation time.
N = 5;
time = zeros(N,1);
labels = cell(N,1);
for i = 1:N
    time(i)=find(t >= (i-1)*t(end)/(N-1),1);
    %time(i)=numel(t-n+i); %this is to the see last steps.
    labels{i}= ['t = ' num2str(t(time(i))) ' s'];
end

if strcmp(plots,'all plots')
    ...
    hold on
    for i = 0:N-1
       plot(x_neg,result.cssurf(time(i+1),neg),'Color',ColOrd(1+mod(i,7),:))
       plot(x_pos,result.cssurf(time(i+1),pos),'Color',ColOrd(1+mod(i,7),:))
    end
    ...
    hold on
    for i = 0:N-1
       plot(x,result.cl(time(i+1),:),'Color',ColOrd(1+mod(i,7),:))
    end
    ...
    hold on
    for i = 0:N-1
       plot(x_neg,result.ir(time(i+1),neg),'Color',ColOrd(1+mod(i,7),:))
       plot(x_pos,result.ir(time(i+1),pos),'Color',ColOrd(1+mod(i,7),:))
    end
    ...
    hold on
    for i = 0:N-1
       plot(x_neg,result.il(time(i+1),neg),'Color',ColOrd(1+mod(i,7),:))
       plot(x_sep,repmat(result.i,n(2)),'Color',ColOrd(1+mod(i,7),:))
       plot(x_pos,result.il(time(i+1),pos),'Color',ColOrd(1+mod(i,7),:))
    end
    ...
    hold on
    for i = 0:N-1
       plot(x_neg,result.phis(time(i+1),neg),'Color',ColOrd(1+mod(i,7),:))
       plot(x_pos,result.phis(time(i+1),pos),'Color',ColOrd(1+mod(i,7),:))
    end
    ...
    hold on
    for i = 0:N-1
       plot(r(:,1),result.cs(time(i+1),1:m)) 
    end
    xlabel('r [\mum]')
    ylabel('c_s [mol/m^3] at x=0')

    % cathode solid state potential in r-dimension
    subplot(4,2,8);
    hold on
    for i = 0:N-1
       plot(r(:,3),result.cs(time(i+1),end-m+1:end)) 
    end
    xlabel('r [\mum]')
    ylabel('c_s [mol/m^3] at x=l')
