function [Fk, Hk] = state_space_P2D(x_est, k, P)
...
Fk = zeros(P.nj*3);
bk = zeros(P.nj*3, P.nj*2);

j = 1;
Ds = P.Ds_neg;
Rp = P.Rp_neg;
Fs = [1 0; 0 1/(1+P.dt*30*Ds/Rp/Rp)];
bs = [-3/Rp; -45/2/Rp/Rp];

Fk((j-1)*3+1,1:3:end) = Fl(j,:);
Fk((j-1)*3+2:(j-1)*3+3,(j-1)*3+2:(j-1)*3+3) = Fs;
bk((j-1)*3+1,1:2:end) = bl(j,:);
bk((j-1)*3+2:(j-1)*3+3, (j-1)*2+2) = bs;
...
Hk(1:3) = -[dphis_dcl dphis_dc dphis_dq];
Bs = zeros(size(jn(:,k)'));
...
dphis_dc  = dcss_dc*dphis_dcss;
dphis_dq  = dcss_dq*dphis_dcss;
Hk(P.nj*3-2:P.nj*3) = [dphis_dcl dphis_dc dphis_dq];
