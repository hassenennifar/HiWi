...
P.idx_css = 2; % variable index solid surface concentration
P.idx_phil = 3; % variable index liquid potential
...
P.sto_neg = 0.1;  % initial stochiometric parameter for negative particle
P.sto_pos = 0.96;  % initial stochiometric parameter for positive particle
...
P.htc = 1;  % heat-transfer coefficient with external medium (W/m2K)
P.Cp = 500;  % heat capacity of cell (J/kgK)
P.Tam  = 298.15;  % ambient temperature (K)
P.T  = P.Tam;  % temperature (K)

P.RG = 8e-4; % total resistance in foils, leads, and contacts, ohm-m2
...
P.ne = 2; % number of estimated states

P.lambda = [4.49340945790906417530788092728032208222;
    7.725251836937707164195068933063;
    10.9041216594288998271487027901886838721;
    14.0661939128314734799789656006015649969;
    17.220755271930768739573718925061;
    20.371302959287562845091004405435];

P.nLi_solid_comsol=1.799742512078129;
P.nLi_solid_matlab=P.epss_pos*P.L_pos*P.cs0_pos+P.epss_neg*P.L_neg*P.cs0_neg;%1.799748788839716;
