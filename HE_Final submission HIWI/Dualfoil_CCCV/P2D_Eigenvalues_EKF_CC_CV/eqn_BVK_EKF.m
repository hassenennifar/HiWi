function [g, Jrow] = eqn_BVK_EKF(j,k,P,x,sol)
...
g = -jn(j,k) + (i0/P.F*(exp(eta/RTF2) - exp(-eta/RTF2)))/denominator;
    %+ (-css(j,k) + c(j,k) + Rp/35*8*q(j,k))/(Rp/35/Ds) - jn(j,k);
...
b(P.idx_jn) = + i0/P.F/RTF2*P.F*Rf*(exp(eta/RTF2)+exp(-eta/RTF2))/denominator^2 +1;
    %+ i0/P.F*(exp(eta/RTF2) - exp(-eta/RTF2))/denominator^2 ...
    %    *(-coef/cl(j,k)/RTF2*P.F*Rf*exp(-eta/RTF2));   
