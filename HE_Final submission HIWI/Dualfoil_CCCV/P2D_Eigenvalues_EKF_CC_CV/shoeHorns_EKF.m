function [xknew, nconcflag] = shoeHorns_EKF(xknew,x,k,P)
...
for j = 1:P.nj
    ...
    totLinew = totLinew + fh*clnew(j);
end
factor = P.totLiold_est/totLinew;
factor=1;
for j = 1:P.nj
    clnew(j) = factor*clnew(j);
end

% xknew = zeros(P.nj, P.nx);
% xknew(:,P.idx_cl) = clnew;
% xknew(:,P.idx_phil) = philnew;
% xknew(:,P.idx_css) = cssnew;
% xknew(:,P.idx_il) = ilnew;
% xknew(:,P.idx_jn) = jnnew;
% xknew(:,P.idx_phis) = phisnew;
% 
% xknew = reshape(xknew', [], 1);
